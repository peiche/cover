...
function cover_aesop_setup() {
    ...
    add_theme_support("aesop-component-styles", $components );
    
}
add_action( 'after_setup_theme', 'cover_aesop_setup' );
...
// overriding the quote shortcode in order to force font size in rems
if (!function_exists('aesop_quote_shortcode')){
	...
	function aesop_quote_shortcode($atts, $content = null) {
		...
		// set size
		$size = $atts['size'] ? sprintf('%srem', $atts['size']) : false; // rem, not em
